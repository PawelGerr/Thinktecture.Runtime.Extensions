...
| `ValueMemberAccessModifier`   | `AccessModifier`                 | `Public`            | Access modifier of the generated `Value` property (ad-hoc unions only). Regular unions have no `Value` property. The generated code still reads the property internally (metadata delegates and the `SwitchPartially` default callback), so a non-public value only hides it from the type's consumers |
| `ValueMemberName`             | `string`                         | `"Value"`           | Name of the generated raw-value property (ad-hoc unions only). Rename it to free the `Value` identifier, for example to hand-write your own `Value` property of a different type in the partial part. All generated internal references (metadata delegates and the `SwitchPartially`/`MapPartially` default callback) follow the rename, so serialization keeps working. An invalid or colliding name surfaces as a normal C# compiler error, matching `KeyMemberName` |
| `DefaultValueHandling`        | `UnionDefaultValueHandling`      | `Disallow`          | Controls how `default(TUnion)` is treated (ad-hoc unions only). `Disallow` (default): the default value is invalid (`TTRESG047` on `default`/`new()`, generated members throw). `MapToFirstMember`: `default(TUnion)` represents the first member (T1), fully usable. `MapToFirstMember` requires a struct union (`TTRESG081`) with a stateless T1 (`TTRESG082`), drops the generated `IDisallowDefaultValue` interface and the uninitialized-throw branches, and generates no named accessor for the default state (hand-write one in the partial part). Implement `IDisallowDefaultValue` manually to keep banning the `default` keyword while the default state stays valid at runtime |
