...
public class TTRESG061_ObjectFactoryMustImplementValidateMethod
{
   ...
   [Fact]
   public async Task Should_render_message_with_value_parameter_typed_as_value_type_and_out_item_typed_as_object_type()
   {
      // The rendered message must match the signature accepted by the analyzer and generated by the code fix:
      // the 'value' parameter is the underlying value type and the 'out item' parameter is the object type.
      var code = """
         #nullable enable
         using System;
         using Thinktecture;
      ...
      var expected = Verifier.Diagnostic(_DIAGNOSTIC_ID)
                             .WithLocation(0)
                             .WithMessage("The type 'TestClass' with 'ObjectFactoryAttribute<string>' must implement a static method 'Validate' with signature: static ValidationError? Validate(string? value, IFormatProvider? provider, out TestClass? item)");
