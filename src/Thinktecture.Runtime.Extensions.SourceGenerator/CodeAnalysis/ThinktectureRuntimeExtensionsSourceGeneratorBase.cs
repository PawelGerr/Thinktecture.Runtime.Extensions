...
public abstract class ThinktectureRuntimeExtensionsSourceGeneratorBase : IIncrementalGenerator
{
   ...
   private void GenerateCode(SourceProductionContext context, (Compilation Compilation, ImmutableArray<ThinktectureRuntimeExtensionsStates> States) compilationAndCandidates)
   {
      ...
      foreach (var states in compilationAndCandidates.States.Distinct())
      {
         ...
         try
         {
            ...
            EmitFile(context, type.ContainingNamespace.ToString(), typeName, generatedCode);
         }
         catch (Exception ex)
         {
   ...
   private void EmitFile(SourceProductionContext context, string typeNamespace, string typeName, string? generatedCode)
   {
      if (!String.IsNullOrWhiteSpace(generatedCode))
      {
         context.AddSource($"{typeNamespace}.{typeName}{_generatedFileInfix}_Generated.cs", $@"// <auto-generated />
#nullable enable
{generatedCode}");
      }
   }
