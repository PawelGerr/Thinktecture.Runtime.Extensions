...
public sealed class MultiplyOperatorsCodeGenerator : IInterfaceCodeGenerator
{
   ...
   public void GenerateBaseTypes(StringBuilder sb, InterfaceCodeGeneratorState state)
   {
      ...
      sb.Append(@"
   global::System.Numerics.IMultiplyOperators<").AppendTypeFullyQualified(state.Type).Append(", ").AppendTypeFullyQualified(state.Type).Append(", ").AppendTypeFullyQualified(state.Type).Append(">");

      if (!_withKeyTypeOverloads)
         return;

      sb.Append(@",
   global::System.Numerics.IMultiplyOperators<").AppendTypeFullyQualified(state.Type).Append(", ").AppendTypeFullyQualified(state.KeyMember).Append(", ").AppendTypeFullyQualified(state.Type).Append(">");
   }
