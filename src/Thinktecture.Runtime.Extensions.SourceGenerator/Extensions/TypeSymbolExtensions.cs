...
public static class TypeSymbolExtensions
{
   ...
   public static ImmutableArray<IFieldSymbol> GetEnumItems(this ITypeSymbol enumType)
   {
      return enumType.GetMembers()
                     ...
                     {
                        if (!m.IsStatic || m is not IFieldSymbol field || field.IsConst || field.IsPropertyBackingField())
                        {
                           result = null;
                           return false;
                        }
      ...
      OperationAnalysisContext? reportDiagnostic = null)
   {
      void ReportField(IFieldSymbol field)
      {
         ...
         if (locationOfDerivedType is null)
         {
            ...
            location = field.GetIdentifierSyntax(cancellationToken)?.GetLocation() ?? Location.None;
         }
         else
         {
      ...
      void ReportPropertyMustBeReadOnly(IPropertySymbol property, DiagnosticSeverity? severity = null)
      {
         ...
         if (locationOfDerivedType is null)
         {
            ...
            location = property.GetIdentifierSyntax(cancellationToken)?.GetLocation() ?? Location.None;
         }
         else
         {
      ...
      void ReportPropertyInitAccessorMustBePrivate(IPropertySymbol property)
      {
         ...
         var location = property.GetIdentifierSyntax(cancellationToken)?.GetLocation() ?? Location.None;
