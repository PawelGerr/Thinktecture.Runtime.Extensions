...
{{
   ...
   partial {(isEnumARefType ? "class" : "struct")} {state.EnumType} : IEquatable<{state.EnumType}{state.NullableQuestionMark}>
   {{
...
");
         ...
         {
            sb.Append($@"
         return item.{state.KeyPropetyName};
");
         }
...
");
      ...
      {{
         return _typeHashCode ^ {state.KeyComparerMember}.GetHashCode({state.KeyPropetyName});
      }}
